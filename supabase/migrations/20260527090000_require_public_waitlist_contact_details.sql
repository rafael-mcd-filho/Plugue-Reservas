...
DROP FUNCTION IF EXISTS public.join_public_waitlist(text, text, text, integer, text, text, text, text);
...
DECLARE
  ...
  _normalized_email text := lower(NULLIF(btrim(COALESCE(_guest_email, '')), ''));
  _normalized_birthdate_text text := NULLIF(btrim(COALESCE(_guest_birthdate, '')), '');
  _guest_birthdate_date date;
...
BEGIN
  ...
  IF _normalized_email IS NULL THEN
    RAISE EXCEPTION 'Informe seu email.';
  END IF;
  ...
  IF _normalized_birthdate_text IS NULL THEN
    RAISE EXCEPTION 'Informe sua data de nascimento.';
  END IF;

  IF _normalized_birthdate_text !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' THEN
    RAISE EXCEPTION 'Informe uma data de nascimento valida.';
  END IF;

  BEGIN
    _guest_birthdate_date := _normalized_birthdate_text::date;
  EXCEPTION
    WHEN invalid_datetime_format OR datetime_field_overflow THEN
      RAISE EXCEPTION 'Informe uma data de nascimento valida.';
  END;

  IF _guest_birthdate_date > CURRENT_DATE THEN
    RAISE EXCEPTION 'A data de nascimento nao pode ser futura.';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.join_public_waitlist(text, text, text, integer, text, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.join_public_waitlist(text, text, text, integer, text, text, text, text) TO authenticated;
